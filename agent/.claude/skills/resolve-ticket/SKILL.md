...
```bash
"$GH" issue list --label "needs: agent" --state open --repo SankaiAI/Bidtopus
```
...
"$GH" issue edit NUMBER --remove-label "needs: agent" --add-label "needs: CORRECT_TEAM" --repo SankaiAI/Bidtopus
```
