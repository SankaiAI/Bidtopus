...
You are the **agent** component submitting a ticket to another team.
`$ARGUMENTS` format: first word = target team (`backend` | `frontend` | `contracts` | `human`), remainder = short description.
...
```bash
gh issue create \
  --title "[agent → TARGET] SHORT DESCRIPTION" \
  --label "LABEL" \
...
```bash
gh issue create \
  --title "[agent → human] SHORT DESCRIPTION" \
  --label "needs: human" \
  --repo SankaiAI/outcomeX \
  --body "**From:** agent
