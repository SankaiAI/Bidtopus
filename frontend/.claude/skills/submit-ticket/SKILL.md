...
You are the **frontend** component submitting a ticket to another team.
`$ARGUMENTS` format: first word = target team (`backend` | `agent` | `contracts` | `human`), remainder = short description.
...
```bash
gh issue create \
  --title "[frontend → TARGET] SHORT DESCRIPTION" \
  --label "LABEL" \
...
```bash
gh issue create \
  --title "[frontend → human] SHORT DESCRIPTION" \
  --label "needs: human" \
  --repo SankaiAI/outcomeX \
  --body "**From:** frontend
