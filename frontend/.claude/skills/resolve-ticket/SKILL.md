---
description: Claim and resolve a ticket assigned to the frontend component. Pass the issue number, or omit to list open frontend tickets.
argument-hint: "[issue-number]"
---

You are the **frontend** component claiming and resolving an assigned ticket.
`$ARGUMENTS` = issue number to resolve (optional — if blank, list open `needs: frontend` tickets first).
...
If no number given, list open frontend tickets first then ask which to work on:
```bash
"$GH" issue list --label "needs: frontend" --state open --repo SankaiAI/Bidtopus
```
...
Before claiming, confirm the work lives inside `frontend/`. Ask:
...
"$GH" issue edit NUMBER --remove-label "needs: frontend" --add-label "needs: CORRECT_TEAM" --repo SankaiAI/Bidtopus
```
