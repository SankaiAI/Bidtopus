---
description: Claim and resolve a ticket assigned to the backend component. Pass the issue number, or omit to list open backend tickets.
argument-hint: "[issue-number]"
---

You are the **backend** component claiming and resolving an assigned ticket.
`$ARGUMENTS` = issue number to resolve (optional — if blank, list open `needs: backend` tickets first).
...
Before claiming, confirm the work lives inside `backend/`. Ask:
